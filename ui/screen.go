...
func (s *Screen) Initialize(window *glfw.Window, shutdownGLFWOnDestruct bool) {
	...
	var err error
	s.context, err = vg.NewContext(vg.StencilStrokes | vg.AntiAlias)
	if err != nil {
		panic(err)
	}
...
func (s *Screen) BackgroundColor() vg.Color {
	return s.backgroundColor
}

// SetBackgroundColor() sets the screen's background color
func (s *Screen) SetBackgroundColor(color vg.Color) {
...
func (s *Screen) NVGContext() *vg.Context {
	return s.context
}
