...
func (i *ImagePanel) Draw(self Widget, ctx *vg.Context) {
	...
	for j, image := range i.images {
		...
		imgPaint := vg.ImagePattern(pX+ix, pY+iy, iw, ih, 0, image.ImageID, toF(i.mouseIndex == j, 1.0, 0.7))
		ctx.BeginPath()
		ctx.RoundedRect(pX, pY, thumbSize, thumbSize, 5)
		ctx.SetFillPaint(imgPaint)
		ctx.Fill()

		shadowPaint := vg.BoxGradient(pX-1, pY, thumbSize+2, thumbSize+2, 5, 3, vg.MONO(0, 128), vg.MONO(0, 0))
		...
		ctx.SetStrokeColor(vg.MONO(255, 80))
		ctx.Stroke()
	}
}
