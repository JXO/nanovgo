...
func (s *Slider) Draw(self Widget, ctx *vg.Context) {
	...
	background := vg.BoxGradient(sx, cy-3+1, sw, 6, 3, 3, vg.MONO(0, a1), vg.MONO(0, a2))
	...
	knobShadow := vg.RadialGradient(kx, ky, kr-3, kr+3, vg.MONO(0, 64), s.theme.Transparent)
	ctx.BeginPath()
	...
	knobPaint := vg.LinearGradient(sx, cy-kr, sx, cy+kr, s.theme.BorderLight, s.theme.BorderMedium)
	knobReversePaint := vg.LinearGradient(sx, cy-kr, sx, cy+kr, s.theme.BorderMedium, s.theme.BorderLight)
	...
	ctx.SetFillColor(vg.MONO(150, a3))
	ctx.Stroke()
	ctx.Fill()
}
