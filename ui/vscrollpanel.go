...
func (v *VScrollPanel) Draw(self Widget, ctx *vg.Context) {
	...
	if v.childPreferredHeight > v.h {
		...
		paint := vg.BoxGradient(x+w-12+1, y+4+1, 8, h-8, 3, 4, vg.MONO(0, 32), vg.MONO(0, 92))
		ctx.BeginPath()
		ctx.RoundedRect(x+w-12, y+4, 8, h-8, 3)
		ctx.SetFillPaint(paint)
		ctx.Fill()

		barPaint := vg.BoxGradient(x+y-12-1, y+4+1+(h-8-scrollH)*v.scroll-1, 8, scrollH, 3, 4, vg.MONO(220, 100), vg.MONO(128, 100))
		ctx.BeginPath()
