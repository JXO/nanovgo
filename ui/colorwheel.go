...
func (c *ColorWheel) Color() vg.Color {
	return vg.HSL(c.hue, c.saturation, c.lightness)
}
...
func (c *ColorWheel) Draw(self Widget, ctx *vg.Context) {
	...
	for i := 0; i < 6; i++ {
		...
		a1 := float32(i+1)/6.0*vg.PI*2.0 + aeps
		ctx.BeginPath()
		...
		color1 := vg.HSLA(a0/(vg.PI*2), 1.0, 0.55, 255)
		color2 := vg.HSLA(a1/(vg.PI*2), 1.0, 0.55, 255)
		paint := vg.LinearGradient(ax, ay, bx, by, color1, color2)
	...
	ctx.SetStrokeColor(vg.MONO(0, 64))
	ctx.Stroke()
	...
	ctx.SetStrokeColor(vg.MONO(255, 192))
	ctx.Stroke()

	paint := vg.BoxGradient(r0-3, -5, r1-r0+6, 10, 2, 4, vg.MONO(0, 128), vg.MONO(0, 0))
	ctx.BeginPath()
	...
	// Center triangle
	r := r0 - 6
	sin1, cos1 := sinCosF(120.0 / 180.0 * vg.PI)
	sin2, cos2 := sinCosF(-120.0 / 180.0 * vg.PI)
	ax := cos1 * r
	ay := sin1 * r
	bx := cos2 * r
	by := sin2 * r
	...
	ctx.LineTo(bx, by)
	ctx.ClosePath()
	triPaint1 := vg.LinearGradient(r, 0, ax, ay, vg.HSL(c.hue, 1.0, 0.5), vg.MONO(255, 255))
	ctx.SetFillPaint(triPaint1)
	ctx.Fill()
	triPaint2 := vg.LinearGradient((r+ax)*0.5, ay*0.5, bx, by, vg.MONO(0, 0), vg.MONO(0, 255))
