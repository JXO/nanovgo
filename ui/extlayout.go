...
func (b *ExpandBoxLayout) OnPerformLayout(widget Widget, ctx *vg.Context) {
	...
	for i, child := range widget.Children() {
		if child.Visible() && !child.IsPositionAbsolute() {
			childCount++
			if _, isScroll := child.(*VScrollPanel); !isScroll {
...
func NewExpandListLayout(widths []int, setting ...int) *ExpandListLayout {
	...
	return &ExpandListLayout{
		alignments: [2]Alignment{Minimum, Minimum},
		widths:     widths,
		margin:     margin,
...
func (g *ExpandListLayout) computeSize(widget Widget, ctx *vg.Context) (widths, heights []int, totalWidth, totalHeight int) {
...
func alignment(align Alignment, space, preferredSize int) (offset, size int) {
