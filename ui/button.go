...
func (b *Button) Draw(self Widget, ctx *vg.Context) {
	...
	var gradTop vg.Color
	var gradBot vg.Color
	...
	bg := vg.LinearGradient(bx, by, bx, by+bh, gradTop, gradBot)
	ctx.SetFillPaint(bg)
	ctx.Fill()
	...
	if b.icon > 0 || b.imageIcon > 0 {
		...
		if b.icon > 0 {
			...
			imgPaint := vg.ImagePattern(iconPosX, iconPosY-ih*0.5, iw, ih, 0, b.imageIcon, eOff)
