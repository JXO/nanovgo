...
func (t *TextBox) Draw(self Widget, ctx *vg.Context) {
	...
	bg := vg.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, vg.MONO(255, 32), vg.MONO(32, 32))
	fg1 := vg.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, vg.MONO(150, 32), vg.MONO(32, 32))
	fg2 := vg.BoxGradient(x+1, y+2, w-2, h-2, 3, 4, vg.RGBA(255, 0, 0, 100), vg.RGBA(255, 0, 0, 50))
	...
	ctx.SetStrokeColor(vg.MONO(0, 48))
	ctx.Stroke()
	...
	if t.unitImage > 0 {
		...
		imgPaint := vg.ImagePattern(x+w-xSpacing-unitWidth, drawPosY-unitHeight*0.5,
		...
		unitWidth, _ = ctx.TextBounds(0, 0, t.units)
		ctx.SetFillColor(vg.MONO(255, toB(t.enabled, 64, 32)))
	...
	if t.committed {
		...
		if len(t.preeditText) != 0 {
			...
			ctx.SetStrokeColor(vg.MONO(255, 160))
			ctx.SetFillColor(vg.MONO(255, 80))
		...
		if caretX > 0 {
			...
			ctx.SetStrokeColor(vg.RGBA(255, 192, 0, 255))
...
func (t *TextBox) updateCursor(ctx *vg.Context, lastX float32, glyphs []vg.GlyphPosition) {
...
func (t *TextBox) textIndex2Position(index int, lastX float32, glyphs []vg.GlyphPosition) float32 {
	if index == len(glyphs) {
		return lastX
	}
...
func (t *TextBox) position2CursorIndex(posX, lastX float32, glyphs []vg.GlyphPosition) int {
	cursorIndex := 0
	if len(glyphs) == 0 {
		return 0
	}
